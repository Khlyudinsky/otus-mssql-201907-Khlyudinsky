/*
 Анализируем план запроса - 53% от времени выполнения занимает скан индекса PK_Sales_Invoices.
 По итогам: 
 1. Sales.CustomerTransactions можно убрать, так как таблица не используется в запросе
 2. Warehouse.StockItemTransactions также можно убрать, так как таблица не используется в запросе
 3. CteQuery лучше объединить по индексированному полю Sales.Orders.CustomerID
*/

/*
Итог: 
Таблицу Sales.CustomerTransactions - убрал, таблицу Warehouse.StockItemTransactions - убирать не стал, так как
если ее убрать значительно увеличивается статистика по времени и цпу, sql строит план с парралелизмом, хинт
OPTION(MAXDOP 1) решает проблему, но думаю лучше оставить в запросе таблицу Warehouse.StockItemTransactions.

В итоге еще уменьшилась статистика по цпу и затраченному времени,
...
Table 'Invoices'. Scan count 1, logical reads 11400, physical reads 3, read-ahead reads 11387, lob logical reads 0, lob physical reads 0, lob read-ahead reads 0.
...
(1 row affected)
 ...
 SQL Server Execution Times:
   CPU time = 407 ms,  elapsed time = 716 ms.
SQL Server parse and compile time: 
   CPU time = 0 ms, elapsed time = 0 ms.
...
Completion time: 2019-11-18T14:28:16.3692890+03:00
*/