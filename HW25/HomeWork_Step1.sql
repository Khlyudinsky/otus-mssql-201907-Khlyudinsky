/*
1. Уберем подзапрос по Warehouse.StockItems из WHERE, перенесем его в JOIN.
2. Подзапрос по Sales.OrderLines в условии Where вывнесем в cte запрос.
*/

/*
Итог: - значительно уменьшилось количество логических чтений, но увеличилась статистика по цпу и затраченному времени, 
предполагаю ssd диск мне здесь помог. Особой оптимизации здесь не получилось.
Так как нужно выбрать какой-то вариант для продолжения оптимизаци, то
...
(1 row affected)
 ...
 SQL Server Execution Times:
   CPU time = 859 ms,  elapsed time = 1163 ms.
SQL Server parse and compile time: 
   CPU time = 0 ms, elapsed time = 0 ms.
...
Completion time: 2019-11-18T12:24:17.3125742+03:00
*/